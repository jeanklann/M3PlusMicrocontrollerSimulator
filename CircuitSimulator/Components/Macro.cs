...
namespace CircuitSimulator
{
    public class Macro : Component
    {
        ...
        public Macro(int inputQuantity, int outputQuantity, string name = "Macro", Circuit circuit = null) : base(name,
        ...
        {
            if (pin >= InputInternal.Length || pin < 0)
                throw new Exception("The pin " + pin + " is not a valid input pin");
            return InputInternal[pin].Pins[0];
        }

        public Pin Output(int pin)
        {
            if (pin >= OutputInternal.Length || pin < 0)
                throw new Exception("The pin " + pin + " is not a valid input pin");
        ...
        protected internal override void Execute()
        {
            ...
            if (Enabled)
            {
                for (var i1 = 0; i1 < InputInternal.Length; i1++) InputInternal[i1].Value = Pins[i1].Value;
            ...
            for (var i = 0; i < OutputInternal.Length; i++) Pins[i + InputInternal.Length].Propagate();
        }
