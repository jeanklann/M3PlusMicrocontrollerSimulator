    ...
    public class TokenAnalyzer {
        ...
        public Token NextToken() {
            ...
            if (_nextTokenindex >= _tokens.Count) {
                _nextTokenindex = _tokens.Count - 1;
            }
            return token;
        }
        ...
        private Token NextToken_internal() {
            ...
            do {
                ...
                foreach (var item in Token.Space) {  //It's only space
                    if (item + "" != value) continue;
                    value = "";
                    break;
                }
                foreach (var item in Token.Lineseparator) {  //It's only a blank line
                    if (item + "" != value) continue;
                    value = "";
                    break;
                }
                ...
                foreach (var comment in Token.Comment) {
                    if (comment == value) {    //Comment
                        ...
                        do {
                            ...
                            foreach (var item in Token.Lineseparator) {  //End of comment
                ...
                foreach (var item in Token.Lineseparator) {  //End of the line at the final of the string
                    if (item != value[value.Length - 1]) continue;
                    value = value.Substring(0,value.Length-1);
                    analyzed = true;
                    break;
                }
                foreach (var item in Token.Space) {  //Space character at the final of the string
                    if (item != value[value.Length - 1]) continue;
                    value = value.Substring(0, value.Length - 1);
                    analyzed = true;
                    break;
            ...
            if (value[0] == Token.Ram)
            {
                if (TryParseAnyNumber(value.Substring(1, value.Length - 1), out number)) {
                    if (number < 0 || number > 255) {
                        throw new CompilerError("Erro na linha " + Helpers.CountLines(_program, beginIndex) + ". Valor inválido, os números para este processador devem estar entre 00 e FF em hexadecimal, ou entre 000 e 255 em decimal (00h - FFh e 000d - 255d).");
                    }
                ...
                if (value.Length == 2) {
                    ...
                    throw new CompilerError("Erro na linha " + Helpers.CountLines(_program, beginIndex) + ". Valor de endereçamento inválido.");
                }
            ...
            for (var i = 0; i < value.Length; i++) {
                if(i == 0) {
                    if (!(char.IsLetter(value[i]) || value[i] == '_')) {
                        throw new CompilerError("Erro na linha " + Helpers.CountLines(_program, beginIndex) + ". Os labels devem começar com uma letra ou com underline (_).");
                    }
                }
                if(!(char.IsLetterOrDigit(value[i]) || value[i] == '_')) {
                    throw new CompilerError("Erro na linha " + Helpers.CountLines(_program, beginIndex) + ". Os labels devem ter somente letras, números e undelines (_).");
                }
            }
