...
namespace IDE
{
    public partial class InstructionLogTableForm : Form
    {
                ...
                try
                {
                    lock (dataGridView1)
                    {
                        ...
                        foreach (var instrucao in instrucoes)
                        {
                            ...
                            if (instrucao.ClocksNecessarios.HasValue)
                                valor += $", Clocks necessários: {instrucao.ClocksNecessarios}";
        ...
        private void InstructionLogTableForm_Load(object sender, EventArgs e)
        {
            ...
            {
                dataGridView1.Rows.Add();
                for (var j = 0; j < columns.Count; j++) dataGridView1.Rows[i].Cells[j].ToolTipText = columns[j];
            }
