...
namespace IDE
{
    public partial class Circuito : GLControl
    {
        ...
        private readonly Queue<Keys> _konamiCode = new Queue<Keys>();

        private readonly Keys[] _konamiCodeCorrect =
        {
            Keys.NumPad8, Keys.NumPad8, Keys.NumPad2, Keys.NumPad2, Keys.NumPad4, Keys.NumPad6, Keys.NumPad4,
            Keys.NumPad6, Keys.B, Keys.A
        };

        private readonly Stopwatch _syncronizer = new Stopwatch();
        ...
        public Dictionary<Component, CircuitSimulator.Component> CircuitComponentToDrawComponents =
            new Dictionary<Component, CircuitSimulator.Component>();

        public Dictionary<Wire, CircuitSimulator.Components.Wire> CircuitWireToDrawWire =
            new Dictionary<Wire, CircuitSimulator.Components.Wire>();

        public Color ClearColor;
        public List<Component> Components = new List<Component>();

        public Dictionary<CircuitSimulator.Component, Component> DrawComponentsToCircuitComponent =
            new Dictionary<CircuitSimulator.Component, Component>();

        public Dictionary<CircuitSimulator.Components.Wire, Wire> DrawWireToCircuitWire =
            new Dictionary<CircuitSimulator.Components.Wire, Wire>();
            ...
            while (!UiStatics.WantExit)
                try
                {
                    ...
                    if (UiStatics.Simulador == null || InternalComponents.ControlModule == null ||
                        InsideComponent == null || InsideComponent.Draw.DisplayListHandle !=
                        Draws.Microcontroller.DisplayListHandle ||
        ...
        private void RefreshExtraTerminals()
        {
            ...
            for (var i = 0; i < Wires.Count; i++)
            {
                ...
                if (Wires[i].ToComponent == null) RefreshExtraTerminals_internal(Wires[i].To, Wires[i].RootComponent);
            }
        }
        ...
        private void RefreshExtraTerminals_internal(PointF point, Component root)
        {
            ...
            if (count != 2) ExtraTerminals.Add(new ExtraTerminal(point, root));
        }
        ...
        private void ThreadRun()
        {
            while (_circuit != null && !UiStatics.WantExit)
            {
                ...
                try
                {
                    ...
                    foreach (var item in copy)
                        ...
                        else
                        {
                            ...
                            {
                                ((Keyboard) item).Value = KeyDownByte;
                            }
                ...
                {
                    _circuit.Tick();
                    if (_controlModuleLastClock != InternalComponents.ControlModule.Clock.Value &&
                        InternalComponents.ControlModule.Clock.Value == Pin.High) UpdateInstructionLog();
        ...
        private void UpdateInstructionLog()
        {
            ...
            else
            {
                ...
                {
                    if (InstructionLog.Items[InstructionLog.Items.Count - 1].Eoi) item.Primeira = true;
                }
            }
        ...
        private void ProcessControlModule(ControlModule module)
        {
            if (UiStatics.Simulador != null && UiStatics.Simulador.InternalSimulation)
            {
                ...
                if (InternalComponents.ControlModule.NeedSet) GetValuesToSimulator();
                if (InternalComponents.ControlModule.NeedSet) SetValuesFromSimulator();
            }
        }
        ...
        private void MountCircuit()
        {
            try
            {
                ...
                foreach (var item in Components)
                    ...
                    {
                        CircuitSimulator.Component component = _circuit.AddComponent(new Keyboard());
                        CircuitComponentToDrawComponents.Add(item, component);
                        DrawComponentsToCircuitComponent.Add(component, item);
                    }
                    ...
                    {
                        CircuitSimulator.Component component =
                            _circuit.AddComponent(new Chip("Generc chip of " + item.Type, item.Draw.Terminals.Length));
                        CircuitComponentToDrawComponents.Add(item, component);
                        DrawComponentsToCircuitComponent.Add(component, item);
                ...
                foreach (var item in Wires)
                    ...
                    else if (item.FromComponent != null && item.ToComponent == null)
                    {
                        // from is and to is not a component
                        var wire = CircuitWireToDrawWire[item];
                        var from = CircuitComponentToDrawComponents[item.FromComponent];
                    ...
                    {
                        //from is not and to is a component
                        var wire = CircuitWireToDrawWire[item];
                        var to = CircuitComponentToDrawComponents[item.ToComponent];
                        var tempPins = new List<Pin>();
                        foreach (var item2 in Wires)
                    ...
                    {
                        //from and to is not a component
                        var wire = CircuitWireToDrawWire[item];
        ...
        public override void Refresh()
        {
            ...
            if (MouseProps.Button2Pressed)
                Position = new PointF(Position.X + (MouseProps.LastPosition.X - MouseProps.CurrentPosition.X) / _zoom,
                    Position.Y - (MouseProps.LastPosition.Y - MouseProps.CurrentPosition.Y) / _zoom);
            ...
            if (!foundTerminal) Terminals.HoverComponent = null;
            if (!foundTerminal && !foundComponent)
            {
                ...
                */
                ...
                foreach (var item in Wires)
                {
                    ...
                    {
                        distance =
                            (item.From.X - MouseProps.ToWorld(MouseProps.CurrentPosition, ClientSize, Position, _zoom)
                                 .X) *
                            (item.From.X - MouseProps.ToWorld(MouseProps.CurrentPosition, ClientSize, Position, _zoom)
                                 .X) +
                            (item.From.Y - MouseProps.ToWorld(MouseProps.CurrentPosition, ClientSize, Position, _zoom)
                                 .Y) *
                            (item.From.Y - MouseProps.ToWorld(MouseProps.CurrentPosition, ClientSize, Position, _zoom)
                                 .Y);
                        if (distance <= 4 * 4)
                    ...
                    {
                        distance =
                            (item.To.X - MouseProps.ToWorld(MouseProps.CurrentPosition, ClientSize, Position, _zoom)
                                 .X) *
                            (item.To.X - MouseProps.ToWorld(MouseProps.CurrentPosition, ClientSize, Position, _zoom)
                                 .X) +
                            (item.To.Y - MouseProps.ToWorld(MouseProps.CurrentPosition, ClientSize, Position, _zoom)
                                 .Y) *
                ...
                if (!foundTerminal) Terminals.HoverNotComponent = false;
            }
            ...
            if (MouseProps.Button1Pressed && _circuit == null)
            {
                if (Selected != null)
                {
                    if (_canDrag == false)
                    {
                        //float distance = (Center.X + terminal.X - Point.X) * (Center.X + terminal.X - Point.X) + (Center.Y + terminal.Y - Point.Y) * (Center.Y + terminal.Y - Point.Y);
                        float distance =
                            (MouseProps.LastDownPosition.X - MouseProps.CurrentPosition.X) *
                            (MouseProps.LastDownPosition.X - MouseProps.CurrentPosition.X) +
                            (MouseProps.LastDownPosition.Y - MouseProps.CurrentPosition.Y) *
                            (MouseProps.LastDownPosition.Y - MouseProps.CurrentPosition.Y);
                        if (distance > 25 * 25) _canDrag = true;
                    }
        ...
        private void Render()
        {
            ...
            foreach (var item in Components)
            {
                ...
                for (var i = 0; i < item.Draw.Terminals.Length; i++)
                {
                    ...
                    if (UiStatics.Simulador != null && (UiStatics.Simulador.Running || !UiStatics.Simulador.Stopped) &&
                        CircuitComponentToDrawComponents.ContainsKey(item))
                    {
        ...
        private void Circuito_Resize(object sender, EventArgs e)
        {
            ...
            var orthographic =
                Matrix4.CreateOrthographic(c.ClientSize.Width / _zoom, c.ClientSize.Height / _zoom, 1, 10);
        ...
        private void Circuito_MouseClick(object sender, MouseEventArgs e)
        {
            ...
            {
                if ((MouseProps.LastDownPosition.X - e.X) * (MouseProps.LastDownPosition.X - e.X) +
                    (MouseProps.LastDownPosition.Y - e.Y) * (MouseProps.LastDownPosition.Y - e.Y) < 10 * 10)
                {
                    if (InsideComponent != null && InsideComponent.Draw.DisplayListHandle !=
                        Draws.Microcontroller.DisplayListHandle ||
                        InsideComponent == null)
                    {
                ...
                if (e.Button == MouseButtons.Left)
                {
                    if (!_isDebugMode)
                        if (InsideComponent != null)
                            if (InsideComponent.Type == ComponentType.Microcontroller)
                                return;
        ...
        private void Circuito_MouseUp(object sender, MouseEventArgs e)
        {
            ...
            if (_circuit == null)
            {
                if (Terminals.FromComponent != null)
                {
                    if (Terminals.HoverComponent != null)
                    {
                        if (Terminals.HoverComponent != Terminals.FromComponent ||
                            Terminals.HoverIndex != Terminals.FromIndex)
                            Wires.Add(new Wire(Terminals.FromComponent, Terminals.FromIndex, Terminals.HoverComponent,
                                Terminals.HoverIndex));
                    }
                    else
                    {
                ...
                else if (Terminals.FromNotComponent)
                {
                    ...
                    else
                    {
                        ...
                        point.X = (float) Math.Round(point.X / 5) * 5;
                        point.Y = (float) Math.Round(point.Y / 5) * 5;
                        if ((Terminals.From.X - point.X) * (Terminals.From.X - point.X) +
                            (Terminals.From.Y - point.Y) * (Terminals.From.Y - point.Y) >=
                            10 * 10) Wires.Add(new Wire(Terminals.From, point));
                    }
                }
            }
        ...
        */
        private static bool OnWire(PointF point1, PointF point2, PointF point)
        {
            ...
            //calcula a distancia
            if ((maiorX - menorX) * (maiorX - menorX) + (maiorY - menorY) * (maiorY - menorY) < 5 * 5) return true;
            return false;
        }
        ...
        {
            {
                ...
                if (arr.Length == _konamiCodeCorrect.Length)
                {
                    ...
                    {
                        _isDebugMode = true;
                        MessageBox.Show("Você entrou no modo debug.", "Modo debug", MessageBoxButtons.OK,
            ...
            if (_circuit == null)
            {
                if (!_isDebugMode)
                {
                    if (Selected != null)
                    {
                        if (Selected.RootComponent != null)
                            if (Selected.RootComponent.Type == ComponentType.Microcontroller)
                                return;
                    }
                    else if (SelectedWire != null)
                    {
                        if (SelectedWire.RootComponent != null)
                            if (SelectedWire.RootComponent.Type == ComponentType.Microcontroller)
                                return;
                    }
                }
                ...
                {
                    var r = Interaction.InputBox("Nome do componente a ser criado:", "Criar componente [DEBUG]", "");
                    if (r != null && r != "")
                        switch (r)
                        {
                            case "Input":
                            ...
                            case "Registrer8BitCBuffer":
                                Components.Add(new Component(Draws.Registrer8BitCBuffer, new PointF(0, 0),
                                    InsideComponent));
                                break;
                            case "RomAddresser":
                            ...
                            case "Clock":
                                ...
                                break;
                            default:
                                MessageBox.Show("Componente " + r + " inexistente.", "Erro", MessageBoxButtons.OK,
