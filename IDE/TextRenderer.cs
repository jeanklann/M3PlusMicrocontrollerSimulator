...
namespace IDE
{
    ...
    {
        private static readonly Dictionary<int, TextRenderer> _textRenderers = new Dictionary<int, TextRenderer>();
        private static readonly Font _font = new Font(FontFamily.GenericMonospace, 12);
        private Bitmap _bmp;
        ...
        public static void DrawText(string text, Color color, PointF point, Font font)
        {
            ...
            if (!_textRenderers.ContainsKey(hash))
            {
                ...
                renderer._bmp = new Bitmap(1, 1, PixelFormat.Format32bppArgb);
                ...
                renderer._bmp = new Bitmap(width, height, PixelFormat.Format32bppArgb);
                renderer._gfx = Graphics.FromImage(renderer._bmp);
                ...
                GL.TexSubImage2D(TextureTarget.Texture2D, 0,
                    renderer._dirtyRegion.X, renderer._dirtyRegion.Y, renderer._dirtyRegion.Width,
                    renderer._dirtyRegion.Height,
            ...
            else
            {
                ...
                GL.TexCoord2(0, hmax);
                GL.Vertex2(point.X - width / 2, point.Y - height / 2);
                GL.TexCoord2(wmax, hmax);
                GL.Vertex2(point.X + width / 2, point.Y - height / 2);
                GL.TexCoord2(wmax, 0);
                GL.Vertex2(point.X + width / 2, point.Y + height / 2);
                GL.TexCoord2(0, 0);
